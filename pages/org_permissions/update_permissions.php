<?php
    ...
    if ($checked) {
        global $conn;
        $sql = "INSERT INTO org_permissions (org_id, permission_id) 
                VALUES ($org_id, $permission_id)";
        $result = query($sql);
        if (!$result) {
            die("Insert failed");
        }
        $entity_id = mysqli_insert_id($conn);
        $table_name = "org_permissions";
        ...
        $find_entity_sql = "SELECT id FROM org_permissions WHERE org_id = $org_id AND permission_id = $permission_id";
        ...
        if (!$entity_id) {
            http_response_code(400);
            echo json_encode(["error" => "Quan hệ tổ chức - quyền không tồn tại"]);
            exit;
        }
        $sql = "DELETE FROM org_permissions WHERE id = $entity_id";
        query($sql);
