<?php
    ...
    $resourceTypeName = 'system_role_perrmissions';
    $resourceTypeId = getResourceTypeByName($resourceTypeName)['id'];
    // TODO: chưa xử lý trường hợp quan hệ với chính nó
    // $orgIDQuery = "SELECT org_id FROM system_role_groups WHERE id = " . ($_GET['role_id'] ?? 0);
    ...
    function createHandler($redirectLink, $resourceTypeId){
        ...
        if(!hasPermission($_SESSION['user']['id'], 'Create', null, 'system_role_perrmissions')) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            // Bước 1: insert
            $insertQuery = "INSERT INTO system_role_perrmissions (system_role_id, permission_id) 
