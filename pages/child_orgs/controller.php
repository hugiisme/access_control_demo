<?php 
    ...
    if(!checkVersionMatch($userId, getResourceTypeByName('organizations')['id'])){
    ...
    if (!empty($accessibleResources)) {
        ...
        if (!empty($entityIds)) {
            ...
            $query = " SELECT 
                        ...
                        po.name AS 'Tổ chức cha'
                    FROM organizations o 
                    LEFT JOIN organizations po  ON o.parent_org_id = po.id
                    WHERE 1=1
                    AND o.id IN ($entityIdList) AND o.parent_org_id = $org_id";

            // Map alias tiếng Việt -> cột thật để filter/sort
            $columnMapping = [
                'id'                => 'o.id',
                'Tên tổ chức'  => 'srg.name',
                'Tổ chức cha'   => 'srg.description'
            ];
        }
    } else {
    ...
    if ($canCreate) {
        $buttonList[] = [
            "btn_type" => "Create",
            "label"    => "Tạo tổ chức con mới",
            "btn_url"  => "/pages/child_orgs/form.php?org_id=" . $org_id .  "&redirectLink=" . urlencode($reloadLink)
        ];
    }
