...
<?php 
    ...
    if($mode == "edit"){
        ...
        if(!$resource_id){
            redirect_with_message("error", "Vai trò không tồn tại hoặc bạn không có quyền truy cập.");
            exit;
        }
    } else {
        $resource_id = null;
    ...
    if ($mode == "edit" && !$canEdit) {
       redirect_with_message("error", "Bạn không có quyền chỉnh sửa vai trò này.");
        exit;
    }
    if ($mode == "create" && !$canCreate) {
        redirect_with_message("error", "Bạn không có quyền tạo vai trò mới.");
        exit;
    }
    $formTitle = ($mode == "edit") ? "Chỉnh sửa vai trò" : "Tạo vai trò mới";
    ...
    $form->addField('text', 'name', 'Tên vai trò');
    $form->addField(
        'text',
        'description',
        'Mô tả'
