...
<body>
    <form action="" method="POST" class="form">
        ...
        <div class="form-group">
            <label for="parent-org-id">Tổ chức cha</label>
            <select name="parent_org_id" id="parent-org-id" class="input-select">
                <option value="NULL">NULL</option>
                <?php
                    if ($organizationsResult && mysqli_num_rows($organizationsResult) > 0) {
                        while ($row = mysqli_fetch_assoc($organizationsResult)) {
                            $orgSelected = (!empty($org) && $org['parent_org_id'] == $row['id']) ? 'selected' : '';
                            echo "<option value='" . $row['id'] . "' $orgSelected >" . 
                                htmlentities($row['name']) 
                            . "</option>";
                        }
                    } else {
                        echo "<option value=''>Không có tổ chức</option>";
                    }
                ?>
            </select>
        </div>
